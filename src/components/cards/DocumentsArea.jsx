...
export default function DocumentsArea({ info }) {
  ...
  const getAllDocuments = async () => {
    try {
      const response = await api.docs.listAllDocs();
            ...
            >
              <CardsBox $columns={numOfColumns}>
                {info[typeKey].map((documentItem) => (
                  <DocumentCard key={documentItem.hash}>
                    <JsonEditor
                      data={documentItem.document}
                      rootName={documentItem.document?.name || "Document Data"}
                      viewOnly
                      indent={2}
                      //collapsed={1} // economizar espaço
